...
contract Metanonia is
    ERC20Lockable,
    ERC20Burnable,
    ERC20Mintable,
    Freezable
{
    string constant private _name = "METANONIA";
        ...
        returns (bool success)
    {
        require(
            to != address(0),
            "GAO/transfer : Should not send to zero address"
        );
        ...
        returns (bool success)
    {
        require(
            to != address(0),
            "GAO/transferFrom : Should not send to zero address"
        );
        ...
        _approve(
            ...
            _allowances[from][msg.sender].sub(
                amount,
                "GAO/transferFrom : Cannot send more than allowance"
            )
        );
        success = true;
    }
        ...
        returns (bool success)
    {
        require(
            spender != address(0),
            "GAO/approve : Should not approve zero address"
        );
